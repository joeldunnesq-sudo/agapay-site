...
function amountInWords(cents) {
  ...
  const under1000 = (x) =>
    x < 100 ? under100(x) : `${small[Math.floor(x / 100)]} hundred${x % 100 ? ` ${under100(x % 100)}` : ''}`;
  const words = (x) =>
    x < 1000
      ? under1000(x)
      : x < 1000000
        ? `${under1000(Math.floor(x / 1000))} thousand${x % 1000 ? ` ${under1000(x % 1000)}` : ''}`
        : String(x);
...
function checkSheet(detail) {
  ...
  const copy =
    detail.prints.length > 1 ? `<div class="reprint">REPRINT · ORIGINAL CHECK ${esc(payment.checkNumber)}</div>` : '';
  const rows = detail.applications
    .map(
      (item) =>
        `<tr><td>${esc(item.vendorInvoiceNumber || item.billNumber)}</td><td>${esc(item.billDate)}</td><td>${esc(item.description)}</td><td>${money(item.amountApplied)}</td></tr>`
    )
    .join('');
...
export function printableChecks(details) {
  const title =
    details.length === 1 ? `Check ${esc(details[0].payment.checkNumber)}` : `Payment run · ${details.length} checks`;
