...
// Parish payout diagnostics, monthly reconciliation, and reconciliation close.
...
const reconciliationDatabase = (env) => env.AGAPAY_DB || env.DB || null;
...
async function reconciliationCloseRecord(env, parishId, month) {
  ...
  const raw = reconciliationDatabase(env) ? await d1GetSetting(env, key) : await env.AGAPAY_REGISTRATIONS?.get(key);
  if (!raw) return null;
...
async function saveReconciliationCloseRecord(env, parishId, month, record) {
  ...
  if (reconciliationDatabase(env)) return d1SetSetting(env, key, value);
