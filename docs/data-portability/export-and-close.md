...
| `TAX_EXEMPTION_DOCS`, `NONPROFIT_PRICING_DOCS`, `GIVING_STATEMENTS`, `SACRAMENT_DOCUMENTS` | Include parish-authorized documents; do not expose another donor's independent records | Apply the approved retention classification to each category; sacrament preparation files remain private pastoral records |
