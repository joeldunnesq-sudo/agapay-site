...
Phase 5 completes with `src/worker.js` as an 811-line composition shell. Public intake and liturgical responses live in `src/handlers/public-entry.js`; platform health and Odyssey activation live in `src/handlers/platform-actions.js`; stewardship giving and reporting are divided between `src/handlers/stewardship-giving.js` and `src/handlers/stewardship-reports.js`; scheduled digest implementations live in `src/operations/weekly-email-digests.js`; and the frozen 290-action dependency registry lives in `src/routes/worker-actions.js`. The Worker still owns cross-cutting gates, the nine-router precedence, all sixteen scheduled-job branches in their established order, API fallback, redirects, and static assets. Its three-name public export surface remains unchanged through compatibility re-exports.
...
## My AGAPAY calendar and session boundaries

`public/donor/controllers/calendar.js` owns the liturgical hero, saint modal,
feast highlights, personal services, parish-calendar rendering, and calendar page
loader. This is a verbatim extraction from the Donor app; the classic global
function names and event handlers remain compatible. Pages loading Donor app
code load the controller first. The Donor app's physical budget is reduced to
3,963 lines. The mobile calendar browser test exercises both Koinonia and the
full calendar, including saint-modal controls, ordinary/feast reading groups,
reloads, app resume, and unauthorized-session navigation.

`public/donor/session.js` is the shared owner for session keys, profile storage,
headers, sign-in persistence, and cleanup. It loads synchronously before the
shell, Donor app, and passkey client. Existing Donor and shell functions remain
compatibility bridges. Logout and an account switch now remove the same session,
capability, Learn-plan, navigation, and current-account response-cache data.
Unrelated settings and downloaded media are preserved. The shell starts a single
login navigation when parallel requests reject the session. Loading a page and
background/resume events do not mutate a session.

Shared asset references are generated by `scripts/version-app-assets.mjs` using
canonical-LF SHA-256 content hashes. The registry is in
`scripts/lib/app-asset-versions.mjs`. Add newly extracted shared assets to that
registry and wire their script order explicitly; the versioning script updates
existing references without inserting scripts or changing their loading mode.
