...
`PIPELINE_VERSION = "directory-media-v2"` and `ACCEPTED_PIPELINE_VERSIONS = ["directory-media-v1", "directory-media-v2"]` (`src/directory/media-transform.js`) are the single, centralized source of truth every approval-gate and delivery-gate check consults (`isAcceptedPipelineVersion()`). No accepted-version string is hardcoded anywhere else. The v2 marker records the Photon 0.4.0 decoder/encoder upgrade while retaining delivery of already verified v1 assets. A future security-relevant pipeline change (decoder, encoder, resize algorithm, metadata handling, output format, variant dimensions, crop semantics, color normalization) bumps `PIPELINE_VERSION` and adds the new string to `ACCEPTED_PIPELINE_VERSIONS`; an old version simply ages out of that array only when an explicit security decision requires reprocessing (see `26-phase-2b1-legacy-media-remediation-plan.md`).
