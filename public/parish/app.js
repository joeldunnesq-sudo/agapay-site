// ── STATE ────────────────────────────────────────────────
  ...
  function renderAccountingPayables(pane) {
    ...
    const overview = data.overview || {}, tabs = [['bills','Bills'],['payments','Payments & Checks'],['runs','Payment runs'],['1099','1099 review'],['vendors','Vendors'],['aging','Aging']];
  ...
  const ACCOUNTING_REPORT_LIBRARY = [
    ...
    { id:'expenses', title:'Expense Breakdown', group:'Accounting detail', copy:'Visual expense analysis including automatically posted AGAPAY fees.' }
  ];
  ...
  async function loadAccountingPhaseD() {
    ...
    try {
      ...
      const [overviewRes, vendorsRes, billsRes, agingRes, budgetsRes, paymentsRes, runsRes, tax1099Res, banksRes] = await Promise.all(['/payables/overview','/payables/vendors','/payables/bills','/payables/aging','/budgets','/payables/payments','/payables/payment-runs',`/payables/1099-summary?year=${year}`,'/bank/accounts'].map((path) => fetch(accountingApi(path), { headers: authHeaders() })));
      const [overview, vendors, bills, aging, budgets, payments, runs, tax1099, banks] = await Promise.all([overviewRes,vendorsRes,billsRes,agingRes,budgetsRes,paymentsRes,runsRes,tax1099Res,banksRes].map((res) => res.json().catch(() => ({}))));
      const failure = [[overviewRes,overview],[vendorsRes,vendors],[billsRes,bills],[agingRes,aging],[budgetsRes,budgets],[paymentsRes,payments],[runsRes,runs],[tax1099Res,tax1099],[banksRes,banks]].find(([res]) => !res.ok);
      if (failure) throw new Error(failure[1].message || failure[1].error || 'Parish Accounting is unavailable.');
      accountingData.payables = { overview: overview.overview, vendors: vendors.vendors || [], bills: bills.bills || [], payments: payments.payments || [], paymentRuns: runs.paymentRuns || [], tax1099: tax1099.report || null, bankAccounts: banks.accounts || [], aging: aging.aging || { rows: [], totalDue: 0 } };
  ...
  function setAccountingPayablesView(view) { accountingPayablesView = ['bills','payments','runs','1099','vendors','aging'].includes(view) ? view : 'bills'; renderAccountingPane(); }
  ...
  async function editAccountingBudgetLines(budgetId){const res=await fetch(accountingApi(`/budgets/${encodeURIComponent(budgetId)}`),{headers:authHeaders()}),payload=await res.json().catch(()=>({}));if(!res.ok){alert(payload.message||payload.error);return;}accountingBudgetEditor={budgetId,lines:payload.detail?.lines||[]};renderAccountingPane();}
