// ── STATE ────────────────────────────────────────────────
  ...
  function renderAccountingPayments(pane,data){const overview=data.overview||{};pane.innerHTML=`<div class="acct-kpis"><div><span>Open payables</span><strong>${accountingMoney(overview.openPayables)}</strong></div><div><span>Checks ready</span><strong>${(data.payments||[]).filter(p=>p.status==='approved').length}</strong></div><div><span>Checks posted</span><strong>${(data.payments||[]).filter(p=>p.status==='posted').length}</strong></div></div><div class="acct-subtabs">${[['bills','Bills'],['payments','Payments & Checks'],['vendors','Vendors'],['aging','Aging']].map(([id,label])=>`<button class="${accountingPayablesView===id?'active':''}" onclick="setAccountingPayablesView('${id}')">${label}</button>`).join('')}</div><div class="acct-list-head"><div><span class="acct-kicker">Payment desk</span><h2>Payments & check register</h2></div><button class="acct-primary" type="button" onclick="showAccountingPaymentForm()">Pay bills</button></div><div id="accountingPhaseDForm"></div><div class="acct-table-wrap"><table class="acct-table"><thead><tr><th>Date</th><th>Check</th><th>Vendor</th><th>Bank</th><th>Amount</th><th>Status</th><th>Action</th></tr></thead><tbody>${(data.payments||[]).map(payment=>`<tr><td>${accountingDate(payment.paymentDate)}</td><td><strong>${escapeHtml(payment.checkNumber||payment.referenceNumber||payment.paymentNumber)}</strong>${Number(payment.printCount)>1?'<br><small>Reprinted</small>':''}</td><td>${escapeHtml(payment.vendorName)}</td><td>${escapeHtml(payment.bankAccountName)}</td><td>${accountingMoney(payment.totalAmount)}</td><td><span class="acct-status ${escapeAttr(payment.status)}">${escapeHtml(payment.status)}</span></td><td><div class="acct-row-actions">${payment.status==='approved'?`<button onclick="printAccountingCheck('${escapeAttr(payment.id)}',${Number(payment.printCount)||0})">${payment.printCount?'Reprint':'Print'}</button><button onclick="postAccountingPayment('${escapeAttr(payment.id)}',${payment.version})" ${payment.printCount?'':'disabled title="Print the check before posting"'}>Post</button>`:payment.status==='posted'?`<button onclick="printAccountingCheck('${escapeAttr(payment.id)}',${Number(payment.printCount)||0})">Reprint</button>`:''}${['approved','posted'].includes(payment.status)?`<button onclick="voidAccountingPayment('${escapeAttr(payment.id)}',${payment.version})">Void</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="7">No payments or checks yet.</td></tr>'}</tbody></table></div>`;}
