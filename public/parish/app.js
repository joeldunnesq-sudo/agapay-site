// ── STATE ────────────────────────────────────────────────
  ...
  async function removeDirectoryPersonFromParish(personId, displayName, expectedVersion, householdId = '') {
    ...
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok || payload.ok === false) throw new Error(payload.message || payload.error || 'Unable to remove parish access.');
      setStatus(`${name} no longer has parish directory or Koinonia access. Their account and giving history were preserved.`, 'success');
  ...
  async function openDirectoryPerson(personId) {
    ...
    try {
      ...
      const koinoniaAllowed = parishConnected && access.linked && Boolean(household.id) && verificationStatus === 'current';
      ...
      detail.innerHTML = directoryRecordDetailShell('Person record', person.preferredName || person.legalName || 'Directory person', 'See exactly what this person can access and what other parishioners can see.', `
  ...
  function directoryHouseholdAccountRow(member, householdId, verificationStatus, managerIds = new Set()) {
    ...
    const removal = member.parishConnected ? `<button class="is-danger" type="button" onclick="removeDirectoryPersonFromParish('${escapeAttr(member.id)}','${escapeAttr(name)}','${escapeAttr(member.personVersion || '')}','${escapeAttr(householdId)}')">Remove from parish</button>` : '';
  ...
  let bookstoreCatalogState = { loaded: false, products: [], lowStockProducts: [], countSessions: [], starterCatalog: [] };
  ...
  async function bookstoreGuestCheckoutDownloadSvg() {
    ...
    if (!bookstoreGuestCheckoutQrSvg) { setStatus('Bookstore QR code is not ready yet.', 'error'); return ''; }
  ...
  async function downloadBookstoreGuestCheckoutQrPng() {
    ...
    image.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (!blob) { setStatus('Unable to create the bookstore QR code PNG.', 'error'); return; }
        downloadBlob(bookstoreGuestCheckoutQrFilename('png'), blob);
        setStatus('Bookstore checkout QR code PNG downloaded.', 'success');
      }, 'image/png');
    };
    image.onerror = () => { URL.revokeObjectURL(svgUrl); setStatus('Unable to render the bookstore QR code PNG.', 'error'); };
    image.src = svgUrl;
  }
  ...
  function renderBookstoreCountSessions(sessions = []) {
    ...
    pane.innerHTML = `
      ${draft ? `<button class="bookstore-count-session-row is-draft" type="button" onclick="openBookstoreDraftCount('${escapeAttr(draft.id)}')">
      ...
      ${completed.length ? `<div class="bookstore-count-session-head"><strong>Recent physical counts</strong><small>Expected stock, shelf count, and explanations</small></div>
        <div class="bookstore-count-session-list">${completed.map(session => {
  ...
  function buildBookstoreCountModal() {
    ...
    modal.addEventListener('click', event => { if (event.target === modal) closeBookstoreCountModal(); });
  ...
  function renderBookstoreDraftCount(session) {
    ...
    const products = (bookstoreCatalogState.products || []).filter(product => product.trackInventory !== false && product.variantId);
    ...
    pane.innerHTML = `<p class="bookstore-count-intro">Enter what is physically on the shelf. Any difference from AGAPAY’s expected stock needs its own explanation before this count can close.</p>
      ...
      <form class="bookstore-count-form" data-session-id="${escapeAttr(session.id)}" onsubmit="closeBookstorePhysicalCount(event)">
        ...
        ${products.map(product => `<div class="bookstore-count-row" data-product-id="${escapeAttr(product.id)}" data-variant-id="${escapeAttr(product.variantId)}" data-expected="${Number(product.stockQuantity || 0)}">
  ...
  async function startBookstorePhysicalCount(button) {
    ...
    if (existing) { renderBookstoreDraftCount(existing); return; }
    if (button) { button.disabled = true; button.textContent = 'Starting…'; }
    try {
      const res = await fetch(bookstoreApi('/count-sessions'), { method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/json' }, body: '{}' });
      const data = await res.json().catch(() => ({}));
      if (!res.ok && !(res.status === 409 && data.session)) throw new Error(data.error || 'Unable to start a physical count.');
      const session = data.session;
      bookstoreCatalogState.countSessions = [session, ...(bookstoreCatalogState.countSessions || []).filter(item => item.id !== session.id)];
      renderBookstoreCountSessions(bookstoreCatalogState.countSessions);
  ...
  function openBookstoreDraftCount(sessionId) {
    const session = (bookstoreCatalogState.countSessions || []).find(item => item.id === sessionId && item.status === 'draft');
  ...
  async function closeBookstorePhysicalCount(event) {
    ...
    try {
      ...
      setStatus(error.message, 'error');
      if (button) { button.disabled = false; button.textContent = 'Close physical count'; }
    }
  }
  ...
  function renderBookstoreCompletedCount(session) {
    ...
    pane.innerHTML = `<div class="bookstore-count-summary"><span><strong>${escapeHtml(bookstoreMovementDate(session.completedAt || session.startedAt))}</strong><small>${items.length} item${items.length === 1 ? '' : 's'} counted</small></span><em>Completed</em></div>
      <div class="bookstore-count-completed-list">${items.map(item => {
  ...
  async function openBookstoreClosedCount(sessionId) {
    ...
    try {
      const res = await fetch(bookstoreApi('/count-sessions/' + encodeURIComponent(sessionId)), { headers: authHeaders() });
      const data = await res.json().catch(() => ({}));
  ...
  function renderBookstoreMovementHistory(movements) {
    ...
    pane.innerHTML = movements.map(movement => {
      ...
      const type = movement.movementType === 'sale' ? 'Sale'
        : movement.movementType === 'receiving' ? 'Received stock'
          : movement.movementType === 'physical_count' ? 'Physical count' : 'Manual adjustment';
  ...
  function renderKoinoniaMinistries() {
    ...
    const pendingCount = state.ministries.reduce((sum, item) => sum + (item.requests || []).filter(request => ['submitted','under_review','returned'].includes(request.status)).length, 0);
  ...
  function renderKoinoniaMinistryDetail() {
    ...
    if (!target) return;
    const detail = koinoniaMinistriesState.ministries.find(item => (item.ministry || item).id === koinoniaMinistriesState.selectedId);
    if (!detail) { target.innerHTML = '<div class="koinonia-ministry-empty"><span>✦</span><strong>Select a ministry</strong><p>Choose a team to see its members, add a parishioner, and review requests.</p></div>'; return; }
    ...
    target.innerHTML = `
      ...
      ${editing ? `<form class="koinonia-ministry-editor" onsubmit="updateKoinoniaMinistry(event,'${escapeAttr(ministry.id)}')">
  ...
  async function deleteKoinoniaMinistry(ministryId, ministryName) {
    ...
    try {
      ...
      const previous = koinoniaMinistryImageUrls.get(ministryId); if (previous) URL.revokeObjectURL(previous); koinoniaMinistryImageUrls.delete(ministryId);
      koinoniaMinistriesState = { ...koinoniaMinistriesState, loaded:false, selectedId:'', editingId:'' }; await loadKoinoniaMinistries(true); setStatus(`${ministryName} and its group messages were permanently deleted.`, 'success');
    } catch (error) { setStatus(error.message, 'error'); }
  }

  async function loadKoinoniaMinistries(force = false) {
    if (!currentParish?.parishId || (koinoniaMinistriesState.loaded && !force)) { if (koinoniaMinistriesState.loaded) renderKoinoniaMinistries(); return; }
    const list = document.getElementById('koinoniaMinistryList');
    ...
    try {
      ...
      const details = await Promise.all((payload.ministries || []).map(async ministry => {
        const detailResponse = await fetch(directoryAdminApi('/ministries/' + encodeURIComponent(ministry.id)), { headers: authHeaders(), cache:'no-store' });
        const detailPayload = await detailResponse.json().catch(() => ({}));
        return detailResponse.ok ? detailPayload.ministry : { ministry, participants:[], leaders:[], requests:[] };
      }));
      koinoniaMinistriesState = { ...koinoniaMinistriesState, loaded:true, ministries:details, selectedId:koinoniaMinistriesState.selectedId || details[0]?.ministry?.id || '' };
      renderKoinoniaMinistries();
    } catch (error) {
      if (list) list.innerHTML = `<div class="communications-empty"><strong>Ministries are unavailable</strong><p>${escapeHtml(error.message)}</p><button class="btn btn-gold" type="button" onclick="loadKoinoniaMinistries(true)">Try again</button></div>`;
    }
  }

  function selectKoinoniaMinistry(id) { koinoniaMinistriesState.selectedId = id; koinoniaMinistriesState.editingId = ''; renderKoinoniaMinistries(); }

  function toggleKoinoniaMinistryEditor(ministryId, show) {
    koinoniaMinistriesState.editingId = show ? ministryId : '';
    renderKoinoniaMinistryDetail();
    if (show) requestAnimationFrame(() => document.querySelector('.koinonia-ministry-editor input[name="displayName"]')?.focus());
  }
  ...
  async function updateKoinoniaMinistry(event, ministryId) {
    ...
    try {
      ...
      const response = await fetch(directoryAdminApi('/ministries/' + encodeURIComponent(ministryId)), { method:'PATCH', headers:{ ...authHeaders(), 'Content-Type':'application/json' }, body:JSON.stringify(patch) });
      const payload = await response.json().catch(() => ({}));
      ...
      await loadKoinoniaMinistries(true); setStatus('Ministry details updated.', 'success');
    } catch (error) { setStatus(error.message, 'error'); button.disabled = false; button.textContent = 'Save changes'; }
  }
  ...
  async function createKoinoniaMinistry(event) {
    ...
    try {
      ...
      const response = await fetch(directoryAdminApi('/ministries'), { method:'POST', headers:{ ...authHeaders(), 'Content-Type':'application/json' }, body:JSON.stringify(raw) });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.message || payload.error || 'Unable to create this ministry.');
      form.reset(); toggleKoinoniaMinistryComposer(false); koinoniaMinistriesState.loaded = false; koinoniaMinistriesState.selectedId = payload.ministry?.ministry?.id || '';
      await loadKoinoniaMinistries(true); setStatus('Ministry created and ready for its team.', 'success');
  ...
  async function reviewKoinoniaMinistryRequest(requestId, decision) {
    try {
      const response = await fetch(directoryAdminApi('/reviews/ministry_interest/' + encodeURIComponent(requestId) + '/decision'), { method:'POST', headers:{ ...authHeaders(), 'Content-Type':'application/json' }, body:JSON.stringify({ decision, reviewerNote:decision === 'approve' ? 'Welcomed through Koinonia Ministries.' : 'Reviewed through Koinonia Ministries.' }) });
      const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload.message || payload.error || 'Unable to review this request.');
      koinoniaMinistriesState.loaded = false; await loadKoinoniaMinistries(true); setStatus(decision === 'approve' ? 'Parishioner welcomed to the ministry.' : 'Request reviewed.', 'success');
    } catch (error) { setStatus(error.message, 'error'); }
  }

  async function acknowledgeKoinoniaGeneralInterest() {
    const request = koinoniaGeneralInterestRequest(); if (!request) return;
    activeParishFeatureRequest = request; await dismissParishFeatureRequest(false); parishFeatureRequests = parishFeatureRequests.filter(item => item.featureId !== 'ministry-service'); renderKoinoniaMinistries();
  }

  async function saveKoinoniaCalendar(event) {
    event.preventDefault(); const form = event.currentTarget; const button = form.querySelector('button[type="submit"]'); const status = document.getElementById('koinoniaCalendarStatus'); const value = document.getElementById('koinoniaCalendarUrl')?.value.trim() || '';
    button.disabled = true; if (status) status.textContent = 'Checking and saving…';
    try {
  ...
  function renderKoinoniaOverview() {
    ...
    if (calendarInput && document.activeElement !== calendarInput) calendarInput.value = currentParish?.koinoniaCalendarUrl || '';
    if (calendarStatus) calendarStatus.textContent = currentParish?.koinoniaCalendarUrl ? 'Public calendar link saved.' : 'No calendar sync link saved.';
  ...
  function parishPrayerStatusLabel(status) {
    return ({ pending:'Awaiting review', active:'Active', answered:'Answered', flagged:'Reported', declined:'Declined', archived:'Archived' })[status] || contentCategoryLabel(status);
  }
  ...
  function filteredParishPrayerRequests() {
    ...
    if (prayerAdminState.filter === 'review') return rows.filter(item => ['pending','flagged'].includes(item.status) || Number(item.reportCount || 0) > 0);
    if (prayerAdminState.filter === 'active') return rows.filter(item => item.status === 'active' && item.visibility === 'parish_members');
    if (prayerAdminState.filter === 'clergy') return rows.filter(item => item.visibility === 'clergy_only' && !['declined','archived'].includes(item.status));
  ...
  function renderParishPrayerRequests() {
    ...
    const setMetric = (id, value) => { const node = document.getElementById(id); if (node) node.textContent = String(Math.max(0, Number(value || 0))); };
    ...
    if (!rows.length) {
      target.innerHTML = '<div class="prayer-admin-empty"><strong>All caught up</strong><p>No prayer requests match this view.</p></div>';
      return;
    }
  ...
  function setParishPrayerFilter(filter, button) {
    ...
    document.querySelectorAll('[data-prayer-admin-filter]').forEach(item => item.setAttribute('aria-pressed', String(item === button)));
    renderParishPrayerRequests();
  }

  async function loadParishPrayerRequests(force = false) {
    if (!currentParish?.parishId || (prayerAdminState.loaded && !force)) { if (prayerAdminState.loaded) renderParishPrayerRequests(); return; }
    const target = document.getElementById('prayerAdminList');
    ...
    try {
      ...
      prayerAdminState = { ...prayerAdminState, loaded:true, requests:payload.requests || [], settings:payload.settings || {}, metrics:payload.metrics || {} };
      renderParishPrayerRequests();
    } catch (error) {
      if (target) target.innerHTML = `<div class="prayer-admin-empty"><strong>Unable to load requests</strong><p>${escapeHtml(error.message)}</p></div>`;
    }
  }

  async function patchParishPrayerRequest(requestId, changes) {
    const response = await fetch(prayerAdminApi('/' + encodeURIComponent(requestId)), { method:'PATCH', headers:{ ...authHeaders(), 'Content-Type':'application/json' }, body:JSON.stringify(changes) });
    const payload = await response.json().catch(() => ({}));
  ...
  async function quickApproveParishPrayer(requestId) {
    ...
    if (!item) return;
    try {
      await patchParishPrayerRequest(requestId, { status:'active', visibility:'parish_members', expectedRevision:item.revision });
  ...
  function openParishPrayerReview(requestId) {
    ...
    document.getElementById('prayerAdminReviewName').textContent = `${item.actualRequesterName || item.requesterName || 'Parishioner'}${item.anonymous ? ' · anonymous to parishioners' : ''}`;
  ...
  async function saveParishPrayerSettings(event) {
    ...
    if (button) { button.disabled = true; button.classList.add('loading'); }
    try {
      const response = await fetch(prayerAdminApi('/settings'), { method:'PATCH', headers:{ ...authHeaders(), 'Content-Type':'application/json' }, body:JSON.stringify({
