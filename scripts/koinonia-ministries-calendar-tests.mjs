...
const googleShareUrl =
  'https://calendar.google.com/calendar/u/0/r?cid=l157ukqb5alh2mvfccdosnkvdo@group.calendar.google.com&pli=1';
const googleIcsUrl =
  'https://calendar.google.com/calendar/ical/l157ukqb5alh2mvfccdosnkvdo%40group.calendar.google.com/public/basic.ics';
assert.equal(
  normalizeKoinoniaCalendarUrl(googleShareUrl),
  googleIcsUrl,
  'Google Calendar subscription pages should normalize to their public ICS feed'
);

let googleRequest;
assert.equal(
  await fetchKoinoniaCalendarIcs(googleShareUrl, async (url, options) => {
...
assert.equal(
  await fetchKoinoniaCalendarIcs('https://calendar.parish.example/events', async (url, options) => {
    nonGoogleRequest = { url, options };
    return new Response(validIcs, { headers: { 'content-type': 'text/calendar' } });
  }),
  validIcs,
  'a non-Google public HTTPS ICS feed should be accepted without an .ics extension'
);
...
for (const unsafeUrl of [
  ...
  await assert.rejects(
    () =>
      fetchKoinoniaCalendarIcs(unsafeUrl, async () => {
...
await assert.rejects(
  () =>
    fetchKoinoniaCalendarIcs(
      'https://calendar.parish.example/feed',
      async () =>
        new Response(null, {
          status: 302,
  ...
  /public HTTPS/,
  'a redirect to an unsafe destination must be rejected'
);

await assert.rejects(
  () =>
    fetchKoinoniaCalendarIcs(
      'https://calendar.parish.example/not-a-calendar.ics',
      async () => new Response('<html>Not a calendar</html>')
    ),
  /did not return an ICS calendar/,
  'an .ics suffix must not substitute for response-content validation'
);

const recurring = parseKoinoniaCalendarIcs(
  `BEGIN:VCALENDAR
BEGIN:VEVENT
UID:liturgy-weekly
DTSTART:20260802T090000
...
assert.ok(
  recurring.filter((event) => event.title === 'Divine Liturgy').length > 4,
  'weekly calendar events should expand into upcoming instances'
);

const workerUtcCalendar = parseKoinoniaCalendarIcs(
  `BEGIN:VCALENDAR
BEGIN:VEVENT
UID:weekly-vigil
...
assert.equal(
  workerUtcCalendar[0].title,
  'Vigil',
  "TZID recurrence times should survive the Worker's UTC runtime until their actual local start time"
);
assert.equal(
  workerUtcCalendar[0].startsAt,
  '2026-08-08T22:00:00.000Z',
  '5 PM America/Chicago should serialize as 10 PM UTC in August'
);
assert.equal(
  workerUtcCalendar.some((event) => event.title === 'Great Canon'),
  false,
  'COUNT-limited recurrences must not continue after their final instance'
);
assert.equal(
  workerUtcCalendar.some((event) => event.startsAt === '2026-08-15T22:00:00.000Z'),
  false,
  'EXDATE should remove excluded recurring instances'
);
...
END:VEVENT
END:VCALENDAR`;
const transfigurationBeforeStart = parseKoinoniaCalendarIcs(
  transfigurationCalendar,
  new Date('2026-08-19T13:00:00.000Z')
);
assert.equal(
  transfigurationBeforeStart.length,
  1,
  'yearly Google Calendar events should expand into the current festal year'
);
assert.equal(
  transfigurationBeforeStart[0].startsAt,
  '2026-08-19T14:00:00.000Z',
  'yearly TZID recurrences should preserve their local start time'
);
assert.equal(
  transfigurationBeforeStart[0].endsAt,
  '2026-08-19T16:00:00.000Z',
  'yearly TZID recurrences should preserve their duration'
);
const transfigurationInProgress = parseKoinoniaCalendarIcs(
  transfigurationCalendar,
  new Date('2026-08-19T14:10:00.000Z')
);
assert.equal(
  transfigurationInProgress.length,
  1,
  'an in-progress recurring service should remain visible until it ends'
);
...
assert.match(
  app,
  /participationType:\s*'member',\s*publish:\s*true/,
  'parish dashboard ministry assignments should publish the private Directory badge'
);
...
assert.match(app, /approvedPublication:\s*visible/);
...
assert.match(app, /method:\s*'PATCH'[\s\S]*Unable to update this ministry/);
...
assert.doesNotMatch(
  ...
  'calendar sync must not block announcements, groups, or other Koinonia content'
);
...
assert.match(
  lifePage,
  />This Week in the Church<[\s\S]*data-calendar-default-view="week"/,
  'the Koinonia landing should own the week calendar'
);
...
assert.match(donorHandler, /connected:\s*false,\s*internal:\s*true,\s*events:\s*commerceEvents/);
assert.match(
  donorHandler,
  /connected:\s*true,\s*internal:\s*true,\s*subscriptionUrl,\s*events:\s*commerceEvents,\s*unavailable:\s*true/
);
...
assert.match(
  donorApp,
  /hasSubscriptionControls[\s\S]*Events published by your parish for the selected week/,
  'the embedded week calendar should not advertise controls that only exist on the full page'
);
...
assert.match(
  donorApp,
  /isAndroid \? \(googleUrl \|\| subscriptionUrl\) : subscriptionUrl\.replace\(\/\^https:\/i, "webcal:"\)/,
  'Android must use a supported HTTPS or Google Calendar URL instead of the unsupported webcal protocol'
);
assert.match(
  donorApp,
  /subscribeButton\.target = isAndroid \? "_blank" : ""/,
  'the Android flow should leave the installed app to complete calendar subscription'
);
assert.match(
  donorApp,
  /googleButton\.hidden = isAndroid \|\| !googleUrl/,
  'Android should not show a duplicate Google Calendar action'
);
...
assert.doesNotMatch(
  donorCalendar,
  /id="parishCalendarWeekView"|id="parishCalendarMonthView"/,
  'the full calendar should remain month-only'
);
assert.match(
  donorCalendar,
  /id="calendarUpcomingFeast"/,
  'the full calendar should show the next feast before the annual timeline'
);
assert.match(
  donorCalendar,
  /<details class="cal-festal-year" id="calendarFestalYear">/,
  'the full festal year should be collapsed behind an accessible disclosure'
);
assert.match(
  donorApp,
  /const upcomingFeast = \[\.\.\.highlighted, \.\.\.highlightsForYear\(year \+ 1\)\]/,
  'the upcoming feast should roll into the next civil year when necessary'
);
assert.match(
  donorApp,
  /View the full \$\{year\} festal year/,
  'the feast disclosure should identify the full festal year'
);
assert.doesNotMatch(
  donorCalendar,
  /id="saintPreviewCard"/,
  'the full calendar should keep saint lives in the compact hero chip instead of a second Saint of the Day card'
);
assert.match(
  donorCalendar,
  /id="donorSaintModal"[\s\S]*?id="donorSaintModalBody"[\s\S]*?Orthocal\.info/,
  'the full calendar saint chip should open a complete saint-life dialog'
);
assert.match(
  donorStyles,
  /:is\(\.donor-parish-life-page, \.donor-calendar-page\) \.cal-saint-chip \{/,
  'the full calendar saint-life action should share the styled Koinonia pill'
);
assert.doesNotMatch(
  donorCalendar,
...
assert.match(
  donorStyles,
  /\.donor-calendar-page \.calendar-liturgical-hero \.cal-today-row \{ display: block; \}/,
  'the full calendar hero should use the same single text column as Koinonia'
);
assert.match(
  donorStyles,
  /\.donor-calendar-page \.calendar-liturgical-hero \.cal-date-badge \{[\s\S]*?position: absolute;[\s\S]*?top: 0;[\s\S]*?right: 0;/,
  'the full calendar church date should use the compact top-right cutout'
);
assert.match(
  donorStyles,
  /\.donor-calendar-page \.calendar-liturgical-hero \.cal-today-title \{ font-size: 27px;/,
  'the full calendar saint title should stay compact even on long commemorations'
);

console.log(
  'PASS - Koinonia ministries management, full parish calendar, and public ICS subscription are wired end to end'
);
