...
const backendSources = worker + core + stripeConnect + adminHandler + donorHandler + parishHandler + parishReconciliationHandler + parishNotifications + stripeFees + stripeHandler + parishInterestHandler;
...
for (const enforcement of [
  ...
  assert.ok(backendSources.includes(enforcement), `backend should enforce tier access: ${enforcement}`);
}
