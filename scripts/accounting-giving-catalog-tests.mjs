...
assert.deepEqual(
  benevolenceUpgrade.registration.campaigns,
  [],
  'adding the Benevolence Fund must not create a campaign'
);
assert.equal(
...
db.exec(
  'CREATE TABLE giving_funds(id TEXT PRIMARY KEY,parish_id TEXT,name TEXT,code TEXT,is_default INTEGER DEFAULT 0,sort_order INTEGER DEFAULT 0,UNIQUE(parish_id,code))'
);
  ...
  JSON.stringify({
    funds: [
      { id: 'general', name: 'General Fund', restrictionType: 'unrestricted' },
      {
        ...
        accountNumber: 'GIV-507346B3',
        accountingFundId: 'fund_operational_507346b39f44e02ecd15',
...
db.prepare('INSERT INTO giving_funds(id,parish_id,name,code,is_default,sort_order) VALUES(?,?,?,?,?,?)').run(
  'legacy-stewardship',
  'st-fiacre',
  'General Stewardship',
  'stewardship',
...
db.prepare('INSERT INTO donor_offerings(id,parish_id,updated_at,data) VALUES(?,?,?,?)').run(
  'legacy-gift',
  'st-fiacre',
  '2026-01-01',
  JSON.stringify({ giftType: 'stewardship', fund: 'stewardship' })
);
db.prepare('INSERT INTO donor_offerings(id,parish_id,updated_at,data) VALUES(?,?,?,?)').run(
  'off_jul_stew_2026',
  'st-fiacre',
  ...
  JSON.stringify({
    ...
    giftAmountCents: 15000,
    amountCents: 15000,
    chargeCents: 15000,
    parishNetCents: 15000,
  })
);
db.prepare('INSERT INTO donor_offerings(id,parish_id,updated_at,data) VALUES(?,?,?,?)').run(
  'off_jul_campaign_2026',
  'st-fiacre',
  '2026-07-02',
  JSON.stringify({
    donorName: 'Joel Dunn',
...
db.prepare('INSERT INTO household_pledges(donor_email,parish_id,fiscal_year,target_amount_cents) VALUES(?,?,?,?)').run(
  'joeldunnesq@gmail.com',
  'st-fiacre',
  2026,
  250000
);
...
const correctedRegistration = JSON.parse(
  db.prepare("SELECT data FROM registrations WHERE parish_id='st-fiacre'").get().data
);
assert.equal(
  correctedRegistration.funds[1].restrictionType,
  'donor_restricted_temporary',
  'St. Fiacre Benevolence must be restricted'
);
for (const expected of STEWARDSHIP_FUND_DEFAULTS) {
  ...
  if (expected.id === 'general') {
    assert.equal(
      correctedFund.accountingFundId,
      'fund_general',
      'General Operating must use the system accounting fund'
    );
  } else {
    assert.match(
      correctedFund.accountingFundId,
      /^fund_(?:giving|operational)_/,
      `${expected.name} must preserve its accounting link`
    );
  }
}
assert.ok(
  !correctedRegistration.funds.some((fund) => fund.id === 'alms'),
  'Poor Box / Alms must not remain a separate fund'
);
assert.equal(
  correctedRegistration.funds.filter((fund) => fund.id === 'benevolence-fund').length,
  1,
  'Benevolence must be the single alms fund'
);
assert.ok(
  !correctedRegistration.funds.some((fund) => fund.id === 'stewardship'),
  'General Stewardship must not remain a separate fund'
);
assert.ok(
  !correctedRegistration.funds.some((fund) => fund.id === 'campaign'),
  'Campaign / Appeal must not remain a generic fund'
);
assert.equal(
  correctedRegistration.funds.find((fund) => fund.id === 'general')?.accountingFundId,
  'fund_general',
  'stewardship must use the canonical General Operating accounting fund'
);
assert.equal(
  JSON.parse(db.prepare("SELECT data FROM donor_offerings WHERE id='legacy-gift'").get().data).fundId,
  'general',
  'historical stewardship donations must normalize to General Operating'
);
assert.equal(
  db.prepare("SELECT COUNT(*) count FROM giving_funds WHERE code IN ('stewardship','campaign')").get().count,
  0,
  'legacy central catalog rows must be retired'
);
assert.equal(
  correctedRegistration.feastCampaigns[0].destinationFundId,
  'benevolence-fund',
  'existing feast campaigns must default to Benevolence'
);
assert.equal(
  db
    .prepare(
      `SELECT SUM(CAST(json_extract(data,'$.amountCents') AS INTEGER)) total
  FROM donor_offerings WHERE parish_id='st-fiacre' AND json_extract(data,'$.campaignId')='alms'`
    )
    .get().total,
  557500,
  'St. Fiacre roof demo gifts must total 55.75% of the goal'
);
assert.equal(correctedRegistration.campaigns[0].giftCount, 8, 'St. Fiacre roof campaign must display eight demo gifts');
assert.equal(
  JSON.parse(db.prepare("SELECT data FROM donor_offerings WHERE id='off_jul_stew_2026'").get().data).giftAmountCents,
  150000,
  "Joel's demo stewardship gift must be $1,500"
);
const receivedDemo = db
  .prepare(
    `SELECT
    COUNT(*) count,
  ...
  WHERE id LIKE 'demo_st_fiacre_2026_received_%'`
  )
  .get();
...
assert.equal(
  db
    .prepare(
      `SELECT SUM(CAST(json_extract(data,'$.giftAmountCents') AS INTEGER)) total
  FROM donor_offerings WHERE id LIKE 'demo_st_fiacre_2026_received_%'`
    )
    .get().total,
  3304000,
  'nudge setup must not change total received'
);
const nudgeDemo = db
  .prepare(
    `WITH stewardship AS (
  ...
  WHERE p.parish_id='st-fiacre' AND p.fiscal_year=2026
    AND COALESCE(s.given_cents,0) < ROUND(p.target_amount_cents * 117.0 / 365.0)`
  )
  .get();
...
assert.equal(
  db
    .prepare(
      `SELECT target_amount_cents FROM household_pledges
  WHERE donor_email='joeldunnesq@gmail.com' AND parish_id='st-fiacre' AND fiscal_year=2026`
    )
    .get().target_amount_cents,
  600000,
  "Joel's demo pledge must qualify for the nudge preview"
);
const joelGiversNudge = db
  .prepare(
    `SELECT
  ...
  FROM donor_offerings
  WHERE parish_id='st-fiacre'
    AND lower(COALESCE(json_extract(data,'$.donorEmail'),''))='joeldunnesq@gmail.com'`
  )
  .get();
assert.equal(
  joelGiversNudge.last_gift_at,
  '2026-06-11T10:00:00.000Z',
  "Joel's latest locally seeded demo gift should be moved into June"
);
...
const firstMerge = mergeStewardshipFundsIntoRegistration({
  funds: [{ id: 'general', name: 'General Operating Fund' }],
});
assert.equal(
  firstMerge.added.length,
  STEWARDSHIP_FUND_DEFAULTS.length - 1,
  'activation must retain General Operating and add the remaining reporting funds'
);
...
assert.equal(
  secondMerge.registration.funds.length,
  firstMerge.registration.funds.length,
  'reconciliation must not duplicate funds'
);
...
assert.equal(
  patronalMerge.registration.feastCampaigns[0].enabled,
  true,
  'the parish feast campaign must be on by default'
);
assert.equal(
  patronalMerge.registration.feastCampaigns[0].patronal,
  true,
  'the parish feast campaign must retain its patronal identity'
);
const legacyMerge = mergeStewardshipFundsIntoRegistration({
  funds: [
...
assert.equal(
  legacyMerge.registration.funds.filter((fund) => fund.id === 'benevolence-fund').length,
  1,
  'legacy alms must merge into Benevolence'
);
assert.ok(
  !legacyMerge.registration.funds.some((fund) => fund.id === 'alms'),
  'legacy alms ID must not survive reconciliation'
);
const duplicateGeneralMerge = mergeStewardshipFundsIntoRegistration({
  funds: [
...
assert.deepEqual(
  duplicateGeneralMerge.registration.funds
    .map((fund) => fund.id)
    .filter((id) => ['general', 'stewardship', 'campaign'].includes(id)),
  ['general'],
  'General Stewardship and generic Campaign / Appeal must collapse into the canonical General Operating fund'
);

const fundColumns = db
  .prepare('PRAGMA table_info(accounting_funds)')
  .all()
  .map((row) => row.name);
for (const column of [
  'giving_source_type',
...
const accountColumns = db
  .prepare('PRAGMA table_info(accounting_accounts)')
  .all()
  .map((row) => row.name);
...
db.prepare(
  ...
  VALUES('fund_general','GENERAL','General Operating Fund','unrestricted','Parish operations',1,1,1)`
).run();
db.prepare(
  `INSERT INTO accounting_funds
  (id,code,name,restriction_type,purpose,is_default,is_active,is_system,giving_source_type,giving_source_id,giving_enabled)
  VALUES('fund_general_legacy','OLD-GENERAL','General Operating Fund (Legacy)','unrestricted','Legacy',0,0,0,'fund','general',0)`
).run();
...
db.prepare(
  `INSERT INTO accounting_funds
  (id,code,name,restriction_type,purpose,is_default,is_active,is_system,giving_source_type,giving_source_id,giving_enabled)
  VALUES('fund_catalog_old','OLD-CATALOG','Old catalog fund','unrestricted','Legacy',0,0,0,'fund','catalog-test',0)`
).run();
db.prepare(
  `INSERT INTO accounting_funds
  (id,code,name,restriction_type,purpose,is_default,is_active,is_system)
  VALUES('fund_catalog_linked','LINKED-CATALOG','Linked catalog fund','unrestricted','Linked',0,1,0)`
).run();
await accountingFund(asyncDb, {
  sourceType: 'fund',
...
assert.equal(
  db
    .prepare("SELECT id FROM accounting_funds WHERE giving_source_type='fund' AND giving_source_id='catalog-test'")
    .get().id,
  'fund_catalog_linked',
  'an explicitly linked ledger fund must reclaim its publishing identity without deleting the historical fund'
);
assert.ok(
  db.prepare("SELECT id FROM accounting_funds WHERE id='fund_catalog_old'").get(),
  'historical ledger funds must remain intact'
);
...
assert.equal(
  upgradedFund.accountingFundId,
  linkedReserve.accountingFundId,
  'upgrade/save must reuse the ledger identity'
);
...
assert.equal(
  db
    .prepare(
      "SELECT COUNT(*) AS total FROM accounting_funds WHERE giving_source_type='fund' AND giving_source_id='mission-reserve'"
    )
    .get().total,
  1
);
...
assert.match(
  wiring,
  /let id = text\(accountingFundId\)/,
  'catalog publishing must preserve an existing ledger fund ID'
);
...
assert.match(
  wiring,
  /return "fund_general"/,
  'stewardship accounting wiring must use the system General Operating fund'
);
assert.match(
  wiring,
  /stewardshipGift \? "general"/,
  'new stewardship source events must resolve to the General Operating identity'
);
assert.match(
  wiring,
  /campaignGift = Boolean\(campaignId\) && !festalAlmsGift/,
  'feast appeals must post to their chosen fund instead of creating campaign funds'
);
assert.match(wiring, /WHERE is_system=0 AND is_default=0/, 'Funds & Alms must retire parallel accounting-only funds');
assert.match(
  wiring,
  /releaseConflictingGivingIdentity/,
  'catalog saves must release stale unique publishing identities before republishing'
);
...
assert.equal(
  ...
  false,
  'computed campaign progress must not turn an ordinary settings save into an accounting catalog save'
);
assert.equal(
  givingCatalogChanged(
    {
...
assert.match(
  stewardship,
  /giftType'\), 'stewardship'\)\) IN \('stewardship','general'\)/,
  'pledge nudges must count stewardship/general gifts only'
);
assert.match(
  parish,
  /\["stewardship", "general"\]\.includes/,
  'My AGAPAY must use the same stewardship/general gift types'
);
assert.doesNotMatch(
  donorApp.match(/async function loadDonorDashboardPage\(\)[\s\S]*?\n}/)?.[0] || '',
  /readDonorCache\("dashboard"\)|renderDonorDashboardPayload\(cached/,
  'personalized dashboard and Give + entitlements must wait for live data'
);
assert.match(
  donorApp,
  /function updateQuickGiveLinks\(parish, \{ syncGivingTier = true \} = \{\}\)[\s\S]*if \(syncGivingTier\) updateGivingTierTiles\(parish\)/,
  'Quick Give must wait for current dashboard data before painting upgrade state'
);
assert.match(
  donorApp,
  /clearDonorCache\("dashboard"\)/,
  'failed dashboard refreshes must discard stale financial cache'
);
assert.match(worker, /f\.reportCode \|\| f\.id/, 'reporting aliases must not create duplicate accounting funds');
assert.match(
  worker,
  /IN \('stewardship','general','general stewardship','general operating fund'\)[\s\S]*THEN 'general'/,
  'legacy and new stewardship reporting must aggregate into General Operating'
);
assert.match(
  parish,
  /isGeneralStewardship\s*\?\s*"General Operating Fund"/,
  'checkout must store stewardship donations in General Operating'
);
assert.match(
  parish,
...
assert.doesNotMatch(
  ...
  'worker must not maintain a parallel default-fund list'
);
assert.doesNotMatch(
  stewardship,
  /const defaults = \[\s*\{ name: "General Stewardship"/,
  'webhook must not maintain a parallel default-fund list'
);
assert.ok(
  parishDashboardHandler.indexOf('synchronizeGivingCatalogWithAccounting') <
    parishDashboardHandler.indexOf('saveRegistrationRecord(env, found.key, updated, current)'),
  'accounting catalog must save before the central registration'
);
...
assert.match(
  app,
  /onclick="editGivingOption\('fund',\$\{row\.index\}\)"/,
  'fund rows in Active giving options must expose inline editing'
);
assert.match(app, /class="options-summary-action"/, 'active-fund edit buttons must sit in the far-right action column');
assert.match(
  app,
  /function updateFeastCampaignFund\b/,
  'Major Feast Alms campaigns must expose a destination-fund selector'
);
assert.match(
  app,
  /destinationFundId:\s*'benevolence-fund'/,
  'new Major Feast Alms campaigns must default to Benevolence'
);
assert.match(app, /Parish feast day/, 'Funds & Alms must label the feast configured in Parish Settings');
assert.doesNotMatch(
  app,
  /<h3 class="option-group-title">Designated funds<\/h3>/,
  'Funds & Alms must not render a duplicate Designated funds card'
);
assert.match(
  app,
  /Benevolence Fund'[^}]+restrictionType:\s*'donor_restricted_temporary'/,
  'new Benevolence funds must default to donor restricted'
);
assert.match(app, /Funds &amp; Alms is the source of truth/);
assert.match(
  app,
  /funds: editableFunds,[\s\S]*campaigns: editableCampaigns,[\s\S]*feastCampaigns: editableFeastCampaigns/,
  'saving another dashboard tab must retain Funds & Alms state'
);
assert.match(
  app,
  /givingCatalogChanged: givingCatalogSnapshot\(\) !== givingCatalogBaseline/,
  'ordinary settings saves must identify unchanged Funds & Alms state'
);
assert.match(
  app,
  /accountingCatalogChanged: accountingCatalogSnapshot\(\) !== accountingCatalogBaseline/,
  'patronal feast settings must not be treated as an Accounting catalog change'
);
assert.match(
  parish,
  /body\.accountingCatalogChanged === true[\s\S]*body\.accountingCatalogChanged === undefined/,
  'the server must honor the explicit Accounting-only catalog signal'
);
assert.match(
  subscriptionCheckout,
  /tier\?\.modules\?\.givingPlus[\s\S]*ensureBenevolenceFundInRegistration/,
  'Give + tier changes must create the canonical Benevolence Fund'
);
assert.match(
  read('migrations/0061_giving_plus_benevolence_fund.sql'),
  /Campaigns remain opt-in[\s\S]*'benevolence-fund'/,
  'existing eligible parishes must receive Benevolence without campaigns'
);
assert.match(
  app,
  /function fallbackCampaigns\(v\) \{ return JSON\.stringify\(Array\.isArray\(v\) \? v : \[\]/,
  'an empty campaign catalog must remain empty until the parish creates a campaign'
);
assert.doesNotMatch(
  adminApp,
  /jsonForTextarea\(reg\.campaigns,\s*\[\{\s*id:\s*['"]campaign['"]/,
  'admin verification must not create a placeholder parish campaign'
);
assert.doesNotMatch(
  app,
  /Advanced edit \(JSON\)|fundsJson|campaignsJson/,
  'Funds & Alms must not expose an internal JSON editor'
);
...
assert.match(
  fundsPanel,
  /<details class="section-card sp-routing-card">[\s\S]*Advanced payment routing[\s\S]*settlementProfilesBody[\s\S]*<div class="section-card">[\s\S]*Funds &amp; Alms/,
  'collapsed payment routing must sit above the Funds & Alms card'
);
assert.doesNotMatch(
  fundsPanel,
  /<details class="section-card sp-routing-card"\s+open/,
  'Advanced payment routing must remain collapsed until opened'
);
assert.doesNotMatch(
  settingsPanel,
  /settlementProfilesBody|Revenue Streams/,
  'Parish Settings must not duplicate payment routing'
);
assert.match(
  app,
  /tab === 'options'[\s\S]*loadSettlementProfilesPanel\(\)/,
  'Funds & Alms must load payment routes on demand'
);
