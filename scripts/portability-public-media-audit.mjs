...
// three inventoried public objects; raw keys are never printed or persisted.
...
const targets={CAMPAIGN_ASSETS:{bucket:'agapay-campaign-assets',base:'https://pub-a8aecb95751f49ac9b078c3e3ed378b8.r2.dev'},ANNOUNCEMENT_ASSETS:{bucket:'agapay-announcement-assets',base:'https://pub-b0974d02d1bf41288b3082849e87f676.r2.dev'},TEACHING_ASSETS:{bucket:'agapay-teaching-assets',base:'https://pub-b6fa9c48d8be43bebaacef7f7ba448e4.r2.dev'}};
if(process.argv.length===2){console.log(JSON.stringify({mode:'plan',command:'node scripts/portability-public-media-audit.mjs --read-only',checks:['r2.dev status','custom domains','inventoried object HEAD/cache metadata'],objectBodies:false,writes:false},null,2));process.exit(0);}
...
for(const [binding,target] of Object.entries(targets)){
  ...
  provider[binding]={r2DevEnabled:dev.includes(`Public access is enabled at '${target.base}'`),customDomains:Number(domains.match(/There are no custom domains/) ? 0 : -1),devStatusSha256:await sha256(dev),domainStatusSha256:await sha256(domains)};
  for(const object of proposal.objects.filter(item=>item.binding===binding)){
    const url=target.base+'/'+object.objectKey.split('/').map(encodeURIComponent).join('/');
    const response=await fetch(url,{method:'HEAD',redirect:'error',signal:AbortSignal.timeout(15000)});
    heads.push({binding,status:response.status,cacheControl:String(response.headers.get('cache-control')||''),etagMatches:response.headers.get('etag')?.replaceAll('"','')===object.etag,urlSha256:await sha256(url)});
  }
}
...
const status=Object.values(provider).every(item=>item.r2DevEnabled&&item.customDomains===0)&&core.reachable===heads.length?'blocked_r2_dev_public':'unexpected_provider_state';
...
console.log(JSON.stringify({status,objectsChecked:core.objectsChecked,reachable:core.reachable,immutableYearCache:core.immutableYearCache,noStore:core.noStore,etagMatches:core.etagMatches,r2DevEnabled:Object.values(provider).filter(item=>item.r2DevEnabled).length,customDomains:0,writes:false},null,2));
if(status!=='blocked_r2_dev_public')process.exitCode=2;
