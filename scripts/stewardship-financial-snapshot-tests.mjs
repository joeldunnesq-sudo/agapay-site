...
assert.equal(
  db.prepare("SELECT contribution_eligible FROM manual_income_entries WHERE id='legacy-cash'").get()
...
assert.match(
  db
    .prepare(
      "SELECT external_assets_json FROM stewardship_authoritative_financial_snapshots WHERE id='legacy-snapshot'"
    )
    .get().external_assets_json,
  /Legacy Endowment/,
  'legacy manually maintained fund records should be preserved as external assets'
);
assert.equal(
  db.prepare("SELECT contribution_eligible FROM manual_income_entries WHERE id='legacy-other'").get()
    .contribution_eligible,
  0,
...
assert.equal((financialHandler.match(/export async function handleStewardshipFinancials/g) || []).length, 1);
