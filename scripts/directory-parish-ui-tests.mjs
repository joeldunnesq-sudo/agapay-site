...
const checks = [
  ...
  ['directory management uses one action queue for submissions and health follow-up', app.includes("fetch(directoryAdminApi('/queue')") && app.includes('Action Queue') && app.includes('directoryManagementQueue(queue, maintenance.actions || {})') && app.includes("queueKind: 'followup'") && app.includes('directoryReviewQueueRows(managementQueue)')],
  ...
  ['maintenance actions open the relevant household or person record', app.includes("item.targetType === 'person' ? 'openDirectoryPerson' : 'openDirectoryHousehold'") && app.includes("followup ? 'Open record' : 'Review'")],
