...
const checks = [
  ...
  ['Directory is gated to the Giving Plus tier module', app.includes("syncDashboardPaywall(document.getElementById('tab-directory'), 'directory', 'Giving Plus', !moduleIncluded('directory'))") && app.includes("const directoryActive = moduleIncluded('directory')")],
  ['Starter retains a visible Directory upgrade path', app.includes("getElementById('nav-directory')?.removeAttribute('hidden')") && app.includes("syncTierRequirementNavigation('directory', 'Giving Plus', directoryActive)")],
