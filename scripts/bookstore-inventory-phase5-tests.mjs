...
sqlite
  .prepare("INSERT INTO commerce_products VALUES ('book_1','parish_1','bookstore','Orthodox Study Bible','active')")
  .run();
sqlite
  .prepare("INSERT INTO commerce_products VALUES ('book_2','parish_1','bookstore','Jordanville Prayer Book','active')")
  .run();
sqlite
  .prepare(
    "INSERT INTO commerce_product_variants VALUES ('variant_1','book_1','parish_1','bookstore','ISBN-1',5,1,'active',?)"
  )
  .run(now);
sqlite
  .prepare(
    "INSERT INTO commerce_product_variants VALUES ('variant_2','book_2','parish_1','bookstore','ISBN-2',3,1,'active',?)"
  )
  .run(now);
...
const unexplained = await closeBookstoreCountSession(
  env,
  'parish_1',
  sessionId,
  {
...
assert.equal(
  sqlite.prepare('SELECT status FROM commerce_inventory_count_sessions WHERE id=?').get(sessionId).status,
  'draft'
);
...
const closed = await closeBookstoreCountSession(
  env,
  'parish_1',
  sessionId,
  {
    items: [
      {
        ...
        variantId: 'variant_1',
        countedQuantity: 4,
        note: 'One damaged copy removed from the shelf',
      },
      { productId: 'book_2', variantId: 'variant_2', countedQuantity: 3, note: '' },
    ],
  },
...
assert.equal(
  sqlite.prepare("SELECT COUNT(*) count FROM commerce_inventory_movements WHERE variant_id='variant_2'").get().count,
  0,
...
assert.equal(
  sqlite.prepare("SELECT stock_quantity FROM commerce_product_variants WHERE id='variant_2'").get().stock_quantity,
  3
);
...
assert.match(
  handler,
  /Add a note explaining the difference for \$\{item\.name\} before closing this count\./,
  'the close handler must use the reconciliation-style required-note register per item'
);
assert.match(
  app,
  /id="bookstoreCountError"/,
...
assert.match(
  dashboard,
  /id="bookstoreCountSessions"/,
  'closed count sessions should be viewable beneath the current bookstore inventory list'
);

console.log(
  'PASS - bookstore Phase 5 requires per-item discrepancy notes, closes atomically, skips matching movements, and preserves closed count detail'
);
