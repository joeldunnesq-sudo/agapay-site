...
const preparationFeature = fs.readFileSync(new URL('../public/parish/features/sacraments/preparation.js', import.meta.url), 'utf8');
...
const checks = [
  ['Sacraments is owned by a registered feature module loaded before the dashboard core', dashboard.indexOf('/parish/feature-registry.js?v=') < dashboard.indexOf('/parish/features/sacraments/pastoral-followup.js?v=') && dashboard.indexOf('/parish/features/sacraments/pastoral-followup.js?v=') < dashboard.indexOf('/parish/features/sacraments/preparation.js?v=') && dashboard.indexOf('/parish/features/sacraments/preparation.js?v=') < dashboard.indexOf('/parish/features/sacraments.js?v=') && dashboard.indexOf('/parish/features/sacraments.js?v=') < dashboard.indexOf('/parish/app.js?') && sacramentsFeature.includes("ParishFeatureRegistry.register('sacraments'") && !coreApp.includes('let sacramentsState')],
  ...
  ['Pastoral follow-up supports parish-wide clergy coverage from personal or dashboard sessions', dashboard.includes('data-sac-tab="follow-up"') && pastoralFollowUp.includes('agapay_identity_session_token') && pastoralFollowUp.includes('return authHeaders()') && app.includes('Cover all clergy') && app.includes('Clergy tickler') && app.includes('Log contact') && app.includes('Schedule visit')],
  ['Preparation explains its workflow and starts each sacrament collapsed', preparationFeature.includes('Preparation workspace') && preparationFeature.includes('AGAPAY privately copies the current template') && preparationFeature.includes('<details class="sac-admin-panel sac-prep-template"') && !/<details class="sac-admin-panel sac-prep-template"[^>]*\sopen(?:\s|>|=)/.test(preparationFeature)],
  ['Standalone repose starts from Directory or Follow-up with an explicit confirmation', directoryFeature.includes('openPastoralReposeFromDirectory') && pastoralFollowUp.includes('Record a repose') && /pastoralMutation\(\s*['"]\/repose['"]/.test(pastoralFollowUp) && pastoralFollowUp.includes("switchTab('sacraments')") && pastoralFollowUp.includes("setSacramentsDashboardTab('follow-up')") && pastoralFollowUp.includes('window.confirm')],
