...
assert.ok(directoryHtml.includes('id="directoryForm"'), 'Directory page should include a directory intake form');
assert.ok(directoryHtml.includes('id="contactEmail"'), 'Directory page should collect contact email addresses');
assert.ok(directoryHtml.includes('Submit a listing'), 'Directory page should identify the Directory intake clearly');
assert.ok(
...
assert.ok(
  registerHtml.includes('startOrganizationRegistration'),
  'Registration page should support organization entry'
);
assert.ok(
  registerHtml.includes('organizationDescription'),
  'Registration page should support values-review descriptions'
);
...
assert.ok(
  backendSources.includes('STRIPE_WEBHOOK_SECRET_CONNECT'),
  'Backend should support a separate Connect webhook signing secret'
);
assert.ok(
  backendSources.includes('handleParishStripeRefresh'),
  'Backend should let parishes refresh Stripe Connect status'
);
assert.ok(backendSources.includes('checkoutFinancials('), 'Backend should centralize donation fee calculations');
assert.ok(
  !backendSources.includes('subscription_data[application_fee_percent]'),
  'Backend should not apply an AGAPAY application fee to recurring donor gifts'
);
assert.ok(
  !backendSources.includes('payment_intent_data[application_fee_amount]'),
  'Backend should not apply an AGAPAY application fee to one-time donor gifts'
);
assert.ok(
  backendSources.includes('AGAPAY does not collect an application fee on donations'),
  'Backend should document that AGAPAY charges no donation platform fee'
);
assert.ok(
  backendSources.includes('Do not add any AGAPAY platform/application fee to bookstore or future commerce checkouts'),
  'Backend should document that Parish Commerce checkout has no AGAPAY application fee'
);
