...
sqlite
  ...
  VALUES ('order_1', 'BK-2026-000145', 'bookstore', 'catalog', 'parish_1', 'product_1', 'variant_1', 4990,
          0, 4990, 0, 0, 4990, 0, 'pending', 'checkout_created', 'cs_1', 'pending', '', ?)`
  )
  .run(now);
sqlite
  .prepare(
  ...
  VALUES ('item_1', 'order_1', 'parish_1', 'bookstore', 'product_1', 'variant_1', 'ISBN-1', 'book',
          'The Orthodox Way', 'The Orthodox Way', 2, 2495, '', '{}', 'physical_pickup', ?, ?)`
  )
  .run(now, now);

await completeCommerceOrderFromStripe(
  env,
  {
    id: 'cs_1',
    amount_total: 4990,
...
assert.equal(
  sqlite.prepare("SELECT stock_quantity FROM commerce_product_variants WHERE id = 'variant_1'").get().stock_quantity,
  3
);

const sharedBody = {
  name: 'The Orthodox Way',
...
const missingReason = await patchBookstoreProduct(
  env,
  'parish_1',
  'product_1',
  { ...sharedBody, stockQuantity: 4 },
  now
);
...
assert.equal(
  sqlite
    .prepare("SELECT COUNT(*) AS count FROM commerce_inventory_movements WHERE movement_type = 'manual_adjustment'")
    .get().count,
  0
);

const adjusted = await patchBookstoreProduct(
  env,
  'parish_1',
  'product_1',
  {
    ...sharedBody,
    stockQuantity: 7,
    stockAdjustmentReason: 'Received new shipment, counted by hand',
  },
  now
);
assert.equal(adjusted.status, 200);
const adjustments = sqlite
  .prepare("SELECT * FROM commerce_inventory_movements WHERE movement_type = 'manual_adjustment'")
  .all();
...
const metadataOnly = await patchBookstoreProduct(
  env,
  'parish_1',
  'product_1',
  {
...
const invalidSale = await patchBookstoreProduct(
  env,
  'parish_1',
  'product_1',
  {
...
const saleUpdate = await patchBookstoreProduct(
  env,
  'parish_1',
  'product_1',
  {
...
assert.equal(
  sqlite.prepare("SELECT sale_price_cents FROM commerce_product_variants WHERE id = 'variant_1'").get()
    .sale_price_cents,
  1795
);
...
const handler = readParishCommerceSource();
const refundBody = handler.slice(
  handler.indexOf('export async function refundCommerceOrderFromStripe'),
  handler.indexOf('export async function disputeCommerceOrderFromStripe')
);
assert.doesNotMatch(
  refundBody,
  /commerce_inventory_movements|stock_quantity/,
  'refunds must not guess whether physical stock returned'
);

console.log(
  'PASS - bookstore Phase 2 logs atomic sale and reasoned adjustment movements while preserving stock on metadata edits'
);
