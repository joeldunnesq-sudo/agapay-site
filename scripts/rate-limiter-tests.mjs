...
import { withFixedRateLimitClock } from './lib/memory-rate-limiter.mjs';

const runtime = new Miniflare(
  convertV4MiniflareOptions({
    modules: true,
    script: `${readFileSync('src/operations/rate-limiter.js', 'utf8')}
export default { fetch() { return new Response('test'); } };`,
    compatibilityDate: '2026-05-25',
    durableObjects: { AGAPAY_RATE_LIMITER: { className: 'RateLimiter', useSQLite: true } },
    cf: false,
    host: '127.0.0.1',
    port: 0,
  })
...
try {
  ...
  await withFixedRateLimitClock(async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 20 }, () => rateLimit(request, env, 'concurrent', { limit: 1, windowSeconds: 300 }))
    );
    ...
    console.log(
      'PASS - real Durable Object concurrent counters, key normalization, bucket isolation, and fail-closed behavior'
    );
  });
} finally {
