...
const unauthorized=await handleAccountingLedger(new Request("https://agapay.app/api/parish/dashboard/st-fiacre/accounting/journals"),{},"st-fiacre");assert.equal(unauthorized.status,401);assert.equal(unauthorized.headers.get("Cache-Control"),"private, no-store");assert.equal(JSON.stringify(await unauthorized.json()).includes("provider"),false);
