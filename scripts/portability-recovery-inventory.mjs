...
const databaseArtifactPattern=/(?:^|[-_])(backup|d1|database|sql)(?:[-_.]|$)/i;
const metadataOnlyRecoveryPattern=/^production-d1-recovery-evidence-\d+$/;
const metadataOnlyRecoveryArtifacts=artifacts.filter(item=>metadataOnlyRecoveryPattern.test(item.name));
const backupArtifacts=artifacts.filter(item=>databaseArtifactPattern.test(item.name)&&!metadataOnlyRecoveryPattern.test(item.name));
...
const natural=JSON.parse(readFileSync(path.join(dir,'natural-expiry-state.json'),'utf8'));
const expiresAfter=Date.parse(natural.expiresAfter), absenceObservedAt=Date.parse(natural.absenceObservedAt);
assert.equal(natural.status,'absence_observed_after_threshold');
assert.ok(Number.isFinite(expiresAfter)&&Number.isFinite(absenceObservedAt)&&absenceObservedAt>=expiresAfter);
assert.ok(natural.observations.some(item=>item.present===true&&Date.parse(item.checkedAt)<expiresAfter));
assert.ok(natural.observations.some(item=>item.present===false&&Date.parse(item.checkedAt)>=expiresAfter));
const core={recentRuns:runs.length,recentSuccessful:runs.filter(run=>run.status==='completed'&&run.conclusion==='success').length,oldestRecentRun:runs.at(-1)?.createdAt||null,newestRecentRun:runs[0]?.createdAt||null,currentGithubArtifacts:artifacts.length,githubDatabaseBackupArtifacts:backupArtifacts.length,githubMetadataOnlyRecoveryArtifacts:metadataOnlyRecoveryArtifacts.length,timeTravel:checks,r2BackupObjects:backup.core.objectsScanned,r2RetentionDays:backup.core.retentionDays,r2LifecycleApplied:backup.status==='applied_and_verified',manualOrOffProviderCopiesVerified:false,naturalLifecycleObserved:true,naturalLifecycleScope:'isolated_staging_r2'};
const status=core.githubDatabaseBackupArtifacts||!checks.filter(c=>c.days<=29).every(c=>c.available)||checks.filter(c=>c.days===31).some(c=>c.available)?'unexpected_recovery_state':'blocked_manual_copy_attestation';
...
console.log(JSON.stringify({status,recentRuns:core.recentRuns,recentSuccessful:core.recentSuccessful,currentGithubArtifacts:core.currentGithubArtifacts,githubDatabaseBackupArtifacts:core.githubDatabaseBackupArtifacts,githubMetadataOnlyRecoveryArtifacts:core.githubMetadataOnlyRecoveryArtifacts,timeTravel:checks.map(({kind,days,available})=>({kind,days,available})),r2BackupObjects:core.r2BackupObjects,manualOrOffProviderCopiesVerified:false,naturalLifecycleObserved:true,naturalLifecycleScope:core.naturalLifecycleScope,writes:false},null,2));
