...
assert.match(server, /\["\/why", "\/give\/#why"\][\s\S]*?\["\/why\.html", "\/give\/#why"\][\s\S]*?\["\/why\/", "\/give\/#why"\]/, "legacy bare Why routes must target the overview anchor");
...
try {
  ...
  for (const retiredPath of ["/give/why", "/give/why.html", "/give/why/", "/why", "/why.html", "/why/"]) {
    ...
    assert.equal(response.headers.get("location"), `${origin}/give/#why`, `${retiredPath} must preserve the #why destination`);
  }
